...
function cumlelereDonustur(dizi, ayrac) {
  let newArr = [...dizi]; //!üç nokta koymadan olmuyor
  return newArr.map((cumle) => cumle.join(ayrac));
}
console.log("görev1");
...
function paragrafOlustur(
  dizi,
  cbcumleKurfonksiyonu,
  cbcumlelereDonusturfonksiyonu
) {
  let newArr = cbcumlelereDonusturfonksiyonu(dizi, " "); //!buradaki ayraç değerini de bir parametre olarak verebilirdim
  let paragraf = cbcumleKurfonksiyonu(
    newArr[1],
    newArr[3],
    newArr[5],
    newArr[7],
